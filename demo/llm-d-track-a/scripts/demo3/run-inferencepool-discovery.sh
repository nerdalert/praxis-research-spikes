...
#
# Praxis discovers endpoints from the InferencePool CRD (v1alpha2)
# instead of static endpoint lists. No static endpoints configured.
...
header() {
  printf '\033[32m[inferencepool] %s\033[0m\n' "$*"
}
...
ensure_praxis_access() {
  local port="${PRAXIS_INFERENCEPOOL_PORT_FORWARD_PORT:-8082}"
  local log_file="/tmp/praxis-inferencepool-port-forward.log"
...
header "Demo 3: InferencePool discovery"
...
say "applying CRD and manifests, waiting for pods"
kubectl apply -f "$MANIFESTS_DIR/crds/inferencepool-crd.yaml" >/dev/null 2>&1 || true
kubectl apply -f "$MANIFESTS_DIR/03-inferencepool-discovery.yaml" >/dev/null
...
say "waiting for discovery to stabilize..."
sleep 5
ensure_praxis_access
...
say "  InferencePool config parsing and endpoint discovery state management."
say "  This KIND demo proves the full Kubernetes API discovery path."
break_line

header "What this demo proves:"
say "  - Praxis discovers endpoints from InferencePool CRD"
say "  - Uses Kubernetes API to list pods matching selectors"
say "  - No static endpoint configuration required"
break_line

header "InferencePool resource:"
...
for i in 1 2; do
  response="$(send_chat_request "fake-model" "discovery test $i")"
  if echo "$response" | grep -q '"fake-model"'; then
    say "  request $i -> ✓ routed via InferencePool discovery"
  else
...
say "  endpoints discovered dynamically via InferencePool selector"
